...
def xccdf_get_items(template, type, items=None):
    ...
    if items == None:
        result.extend(xccdf_get_items(template, type, template.content))
        ...
        for item in items:
            ...
            elif type == oscap.xccdf.XCCDF_PROFILE:
            ...
            # Rule is the base element where we can get checks and definition id's
                ...
                if item.type == oscap.xccdf.XCCDF_RULE:
                    ...
                    result.extend(xccdf_get_items(template, type, item.content))

            elif type == oscap.xccdf.XCCDF_GROUP:
            ...
            if type == oscap.xccdf.XCCDF_CONTENT:
                result.extend(xccdf_get_items(template, type, item))

            if type == oscap.xccdf.XCCDF_ITEM:
...
def item_get_type_str(item):
    ...
    if type == oscap.xccdf.XCCDF_BENCHMARK:
        ...
        return "Profile"
    elif type == oscap.xccdf.XCCDF_RESULT:
        return "TestResult"
    elif type == oscap.xccdf.XCCDF_VALUE:
        return "Value"
    else:
        return "Item"
